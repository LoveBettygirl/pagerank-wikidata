...
{
	cout << "Usage: " << argv << " [-b beta] [-n max_block_size] [-e epsilon] "
		<< "[-t max_iter_count] [input_file_path]" << endl;
	cout << "Default values: " << endl;
	cout << "beta = 0.85" << endl;
	cout << "max block size = 100" << endl;
...
void parse_args(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; )
	{
		...
		else if (strcmp(argv[i], "-n") == 0) // 最大分块大小设置
		{
			int temp;
			if (i + 1 < argc && sscanf(argv[i + 1], "%d", &temp) > 0)
			{
				p.setMaxBlockSize(temp);
				i += 2;
			}
			else
			{
